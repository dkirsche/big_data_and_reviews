 ...
 */
  ...
  */
  ...
  require_once('googlecheckout/googlenotificationhistory.php');
  require_once 'classes/supplierpaidreviewOrder.php';
  require_once 'classes/supplierpaidreview.php';
  require_once 'configDB.php';
    
  define('RESPONSE_HANDLER_ERROR_LOG_FILE', 'suppliers/googleerror.log');
  define('RESPONSE_HANDLER_LOG_FILE', 'suppliers/googlemessage.log');
  ...
  switch($root){
    case "new-order-notification": {
      ...
      $Review->Load($data[$root]['order-summary']['shopping-cart']['merchant-private-data']['VALUE']);
      $Review->externalid=$data[$root]['google-order-number']['VALUE'];
      $Review->status=1;  //order has been submitted for payment.
      $Review->Update();
      break;
    }
	...
	*/
	...
	//if the new state is charged then update the status in the DB
	$newState=$data[$root]['new-financial-order-state']['VALUE'];
	$previousState=$data[$root]['previous-financial-order-state']['VALUE'];
	if($newState!=$previousState && $newState=='CHARGED'){
		$Review=new SupplierPaidReviewOrder();
      		$Review->Load($data[$root]['order-summary']['shopping-cart']['merchant-private-data']['VALUE']);
		$Review->SetPaid();
		$Review->ChooseReviewers(); //setup reviewers to write reviews
		SupplierPaidReview::SendInitEmail(25,$connect); //send alerts to reviewers letting them know they have been chosen to write a review.
